  ...
  <div class="bg-white dark:bg-dark-grey rounded-lg p-1">
    <div class="p-5 pr-3 flex flex-col gap-6 max-h-[90vh] overflow-y-auto ">
      <div class="flex justify-between items-center gap-4">
        ...
        <Dropdown @onClickEdit="editTask" @onClickDelete="deleteTask" target="Задачу" />
      </div>
      <p v-if="boardsStore.getTask?.description" class="text-medium-grey text-sm">{{ boardsStore.getTask.description }}
      </p>
      <div>
        <p class="text-medium-grey dark:text-white text-xs font-bold pb-4">Подзадачи ({{ subtasksCompleted }})</p>
        <div class="flex flex-col gap-2">
          <Subtask v-for="(subtask, index) in boardsStore?.getTask?.subtasks" :key="index" :subtask="subtask" />
        </div>
      </div>
      <div>
        <p class="text-medium-grey dark:text-white text-xs font-bold pb-4">Текуший статус</p>
        <BaseSelect @onClickOption="changeColumn($event)" :value="boardsStore.getCurrentColumn.name" />
      </div>
    </div>
  </div>
</template>
...
const changeColumn = ( index ) => {
  const selectedValue = index.name;
  const selectedIndex = index.index
  const taskId = boardsStore.getTask?.id;
  boardsStore.updateTaskStatus({taskId:taskId, status: selectedValue, index: selectedIndex} )
}
const editTask = () => {
