<template>
  ...
  hover:bg-main-purple hover:bg-opacity-25" :for="subtask?.title">
    <div class="relative">
      <input class="cursor-pointer " v-model="subtask.isCompleted" type="checkbox" :id="subtask?.title" @change="updateSubtaskStatus">
...
<script setup>
import {updateStatusSubTask} from "@/api/responses";
...
const updateSubtaskStatus = async () => {
  const data = {
    ...
    isCompleted: props.subtask.isCompleted ? 1 : 0
  }

  await updateStatusSubTask(data.subtaskId, data.isCompleted )
}
